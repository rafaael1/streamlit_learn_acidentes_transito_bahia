...
wd = df_eta.loc[ (df_eta['Faixa Etária OPS'] == ('Menor 1 ano')) | (df_eta['Faixa Etária OPS'] == ('1 a 4 anos')) | (df_eta['Faixa Etária OPS'] == ('5 a 14 anos')), '2017':'2021' ].sum(axis=0)
...
dfaixa = df_eta.iloc[:-1, :-1].melt(id_vars='Faixa Etária OPS', var_name='Anos', value_name='Valor')
...
# Primeiro gráfico
def first_graph():
# Total de mortalidade por acidentes de trânsito no estado da Bahia
  grafico = go.Figure(
    data=[
      go.Bar(
        x=df_anos,
...
def metrics():
  ...
  with col1:
      st.metric("Total de Óbitos ⚰️", total_acidentes)

  with col2:
      st.metric("Total Masculino 👨", total_masc)

  with col3:
      st.metric("Total Feminino 👩", total_fem)

  with col4:
      st.metric("Total Ignorado 🧑", total_ign)

# Segundo gráfico
def second_graph() :
# Mortalidade por acidentes segundo o gênero
  # Criando o gráfico de barras empilhadas (stacked)
  grafico2 = px.line(x=df_anos, y=df_vitimas).update_traces(showlegend=True, line_color='#9a4c95', name="Total").add_traces(
  
  # Adicionando as barras para cada sexo
  px.bar(dsex_t, x='Anos', y='Valor', color='Sexo', 
      barmode='stack', 
      orientation="v", 
...
# Terceiro gráfico
def third_graph():
  #  Óbitos por acidentes de transporte por cor/raça
...
def quarter_graph():
  # Definindo o gráfico
  qq = px.area(dfaixa, x='Anos', y='Valor', color='Faixa Etária OPS', color_discrete_sequence=px.colors.qualitative.Prism, groupnorm='percent', custom_data=['Faixa Etária OPS', 'Valor'] )

  ## Dados
  print(qq.data[0].hovertemplate)
  print(qq.data[0].customdata)
  qq.update_traces(hovertemplate='Faixa Etária OPS: <b>%{customdata[0]}</b> <br><br>Ano: %{x} <br>Proporção: %{y:4.2f}% <br>Valor Absoluto: %{customdata[1]}<extra></extra>')
  ...
  qq.update_layout(title={
        'text' : 'Total de Óbitos por Faixa Etária OPS*',
        'x' : 0.3 
